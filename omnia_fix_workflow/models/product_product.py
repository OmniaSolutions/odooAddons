...
class product_product(models.Model):
    ...
    @api.multi
    def fix_workflows(self, model='product.product', force_ids=[]):
        brws_obj = self.env[model]
        ids = self.env.context.get('active_ids', [])
        if force_ids:
            ids = force_ids
        for product_id in brws_obj.browse(ids):
            res_id = product_id.id
            wkf_inst = self.get_wkf_instance(model, res_id)
            ...
            if inst_id:
                ...
                if wkf_workitem:
                    ...
                    if not wkf_activity:
                        raise UserError('Cannot find correct activity id')
                ...
                wkf_id = self.get_wkf_id(model)
                inst_id = self.create_wkf_instance(model, wkf_id, res_id)
                expected_activity = self.get_wkf_activity_expected(product_id.state, inst_id.wkf_id.id)
    ...
    @api.model
    def create_wkf_activity(self, wkf_id, inst_id):
        vals = {
            'wkf_id': wkf_id,
            'inst_id': inst_id,
            'state': 'complete',
            }
        new = self.env['workflow.activity'].create(vals)
        return new
