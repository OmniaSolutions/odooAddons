...
class SaleOrder(models.Model):
    ...
    def changeOrderLine(self):
        removed_ids=[]
        sale_order_list = self.env['sale.order.line']
        orderId = self.id or self._origin.id
        if orderId:
            sale_order = self.browse(orderId)
            sale_order_list = sale_order.order_line
            old_ids = sale_order_list.ids
        ...
        for line in self.order_line:
            ...
            for needed_prod_id in line.product_id.needed_children_product_ids:
                ...
                self.update({'order_line': [(0,0, {'parent_sale_line_needed': omnia_id,
        ...
        for old_line in sale_order_list + self.order_line:
