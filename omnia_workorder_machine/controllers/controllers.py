    ...
    def make_login(self, **post):
        ...
        if database and user and pwd:
            try:
                ...
                auth_info = request.env['res.users'].authenticate(database, credential, {})
                if auth_info and auth_info.get('uid'):
                    csrf_token = request.csrf_token()
                    post['csrf_token'] = csrf_token
                    post['user_logged'] = True
                    post['uid'] = auth_info['uid']
                    post['login'] = auth_info.get('login')
            except Exception as ex:
                logging.error(f"Login failed: {ex}")
        return request.render('omnia_workorder_machine.template_workorder_machine', post)
