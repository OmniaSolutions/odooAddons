...
class MrpProductionWCLine(models.Model):
    ...
    def o_do_scrap(self, scrap_qty):
        ...
        for product_id in (self.production_id.move_raw_ids.filtered(
                lambda x: x.state not in ('done', 'cancel')) | self.production_id.move_finished_ids.filtered(
    ...
    def getWorkorder(self,
        ...
        body_row = []
        for workorder_id in self.env['mrp.workorder'].search([
    ...
    def confirm_start_workorder(self, workorder_id):
        for mrp_workorder_id in self.browse([int(workorder_id)]):
            if mrp_workorder_id.state in ['progress']:
                mrp_workorder_id._recordWork(
                    mrp_workorder_id.qty_production - mrp_workorder_id.qty_produced
                )
